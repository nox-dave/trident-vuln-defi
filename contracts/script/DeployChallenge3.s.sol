...
import {Challenge3_Wrapper} from "../src/challenges/Challenge3_Storage/Challenge3_Wrapper.sol";
import {ChallengeFactory} from "../src/ChallengeFactory.sol";

contract DeployChallenge3 is Script {
    function run() external {
        ...
        Challenge3_Wrapper wrapper = new Challenge3_Wrapper();
        
        wrapper.initialize{value: 0.001 ether}();

        console.log("Challenge3_Wrapper deployed at:", address(wrapper));
        console.log("UpgradeableWallet deployed at:", address(wrapper.wallet()));
        console.log("Initialized:", wrapper.initialized());
        console.log("Wallet balance:", address(wrapper.wallet()).balance);
        ...
        try vm.envAddress("CHALLENGE_FACTORY") returns (address factoryAddress) {
            if (factoryAddress != address(0)) {
                ...
                try factory.updateChallengeAddress(3, address(wrapper)) {
                    console.log("Challenge3 registered with factory at:", factoryAddress);
                } catch {
